...
take the formula $y = 1-(a)^x$, where $a < 1$ and $x = residual$. Becuase the lowest
...
p.5 <- ggplot(slopes, aes(x = V1, y = V2)) +
  scale_alpha_identity() +
  ...
  theme(plot.margin = unit(c(0,0,0,0), "cm")) +
  theme(legend.position = "None") +
  labs(title = "Flattest Non-Linear Decay (a = 0.5)")

p.linear <- ggplot(slopes, aes(x = V1, y = V2)) +
  scale_alpha_identity() +
  geom_point(aes(alpha = 1-slope_linear))  +
  labs(x = "", y = "") +
  theme_classic() +
  theme(axis.text = element_blank()) +
  theme(plot.margin = unit(c(0,0,0,0), "cm")) +
...
final_plot <- grid.arrange(p.125, p.25, p.5, p.linear, ncol = 2) 
```
