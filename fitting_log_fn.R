## Run first chunk of E1 report first
...
absent3 <- absent2 %>%
  ...
  mutate(lg88 = (log(1-0.88*mean_obj)/log(1-0.88))) %>%
  mutate(lg91 = (log(1-0.91*mean_obj)/log(1-0.91)))
...
ggplot(data = absent3, aes(x = mean_obj, y = mean_subj)) +
  ...
  geom_smooth(aes(x = mean_obj, y = lg85, colour = "b = 0.85")) +
  #geom_smooth(aes(x = mean_obj, y = lg88, colour = "b = 0.88")) +
  #geom_smooth(aes(x = mean_obj, y = lg91, colour = "b = 0.91")) +
  geom_abline(intercept = 0, slope = 1) +
  xlim(0,1) +
  ylim(0,1) +
  theme_minimal()
